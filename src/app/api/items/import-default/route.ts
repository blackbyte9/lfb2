...
const DEFAULT_ITEMS_FILE = "C:\\Programming\\sbm5\\data\\items.json";
...
export async function POST(request: NextRequest) {
  ...
  try {
    const fileContent = await readFile(DEFAULT_ITEMS_FILE, "utf-8");
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Import aus items.json fehlgeschlagen" },
      { status: 500 },
    );
  }
}
